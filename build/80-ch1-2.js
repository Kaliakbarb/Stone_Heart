...
(function () {
  ...
  Scenes.register("ch2_jalmauyz", {
    ...
    enter: function () {
      this._t = 0; this._crumbleStarted = false;
      stopMusic();
      // onWin sets childrenFreed and re-enters this scene to play the crumble
      // line. Without this guard enter() would reset _phase back to "fight" and
      // the battle would restart forever (the reported bug). childrenFreed is
      // persistent and cleared on a new game, so a fresh run still fights.
      if (flag("childrenFreed")) {
        this._phase = "crumble"; this._started = true;
      } else {
        this._phase = "fight"; this._started = false;
      }
    },
    exit: function () { stopMusic(); },
